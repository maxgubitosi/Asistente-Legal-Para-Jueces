...
try:
    ...
    from post_evaluation.tests.test2 import evaluate_test2_redaction_robustness, load_queries_from_json, DEFAULT_PROMPTS_PATH
    from post_evaluation.tests.test3 import evaluate_test3_content_sensitivity, get_content_sensitive_queries
    from post_evaluation.tests.test4 import evaluate_test4_document_retrieval, load_question_directories
...
Ejemplos de uso:
  ...
  python 9_evaluate.py --test 2 --auto-docker

  # Ejecutar Test 2 (manual con dos backends)
  python 9_evaluate.py --test 2 --original-backend http://localhost:8000 --modified-backend http://localhost:8001

  # Ejecutar Test 4 (precisión de recuperación de documentos)
  python 9_evaluate.py --test 4 --original-backend http://localhost:8000 --sample-size 50

  # Ejecutar todos los tests con Docker automático
  python 9_evaluate.py --all --auto-docker --sample-size 20
...
Requisitos por test:
  ...
  Test 4: Requiere datasets_evaluation/test4/ Y backend RAG original funcionando
        """
    )
    ...
    # Test 2 and 3 prompts
    parser.add_argument('--prompts-path', help='Ruta a archivo JSON con consultas para Test 2 y 3 (Test 4 usa preguntas generadas automáticamente)')
    ...
    for test_num in tests_to_run:
        ...
        try:
            if test_num == 1:                            # Test 1: Robustez de Formato de Citas
                ...
                # Load queries from provided JSON or default
                if args.prompts_path:
                    queries = load_queries_from_json(Path(args.prompts_path))
                else:
                    queries = load_queries_from_json(DEFAULT_PROMPTS_PATH)
            ...
            elif test_num == 4:                            # Test 4: Document Retrieval Accuracy
                # For test4, we expect the structure: test4/2024/questions/{specific,ultra_specific,generic}/
                questions_base_dir = evaluation_data_path / "test4" / "2024" / "questions"
                
                # Verify the questions directory exists
                if not questions_base_dir.exists():
                    raise FileNotFoundError(f"❌ Questions directory not found: {questions_base_dir}")
                
                rag_client = RAGClient(backend_url=args.original_backend)
                
                if not rag_client.check_health():
                    raise ConnectionError(f"❌ No se puede conectar al backend: {args.original_backend}")
                
                result = evaluate_test4_document_retrieval(
                    rag_client=rag_client,
                    questions_base_dir=questions_base_dir,
                    output_dir=output_dir,  # Save directly in timestamped folder
                    max_questions_per_type=args.sample_size
                )
